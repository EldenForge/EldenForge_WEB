<script lang="ts">
	...
	import { fetchArmors } from '$lib/api/items';
	...
	const slots = $derived(
		Array.from(new Set(items.map((a) => a.category).filter(Boolean))).sort()
	);

	const filtered = $derived(
		items
			.filter((a) => !search || a.name.toLowerCase().includes(search.toLowerCase()))
			.filter((a) => !slot || a.category === slot)
			.sort((a, b) => a.name.localeCompare(b.name))
	);

	function negationBadges(a: Armor): string[] {
		const out: string[] = [];
		const phys = (a.dmgNegation || []).find((d) => d.name === 'Phy');
		if (phys && phys.amount) out.push(`Phys ${phys.amount.toFixed(1)}`);
		if (typeof a.weight === 'number') out.push(`${a.weight} wt`);
		return out;
	}
</script>

<svelte:head><title>Armors — Codex — Elden Forge</title></svelte:head>

<div class="max-w-6xl mx-auto px-4 py-8">
	<CodexHeader title="ARMORS" subtitle="Armors" />
	...
	<div class="flex flex-wrap gap-3 mb-5 items-center">
		...
		/>
		<select
			bind:value={slot} aria-label="Filtrer par slot d equipement"
	...
	{:else if filtered.length === 0}
		<p class="text-center text-parchment/40 italic py-12">No armor matches your filters.</p>
	{:else}
